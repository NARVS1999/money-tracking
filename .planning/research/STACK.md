...
**Domain:** Personal expense/income tracker — React Native + Expo (Expo Go workflow), Firebase Firestore, PDF + Excel export
**Researched:** 2026-08-06
**Confidence:** HIGH (versions verified against npm registry, Expo SDK 57 docs, Firebase docs & SDK source; one critical finding contradicts the draft tech-design)
...
npx create-expo-app@latest money-tracking
...
The draft tech-design's core offline assumption ("offline persistence works in Expo Go with the JS SDK + AsyncStorage") is **incorrect**. Verified against three independent primary sources (Aug 2026):

1. **Firebase environments doc** (updated 2026-08-05): React Native column lists Cloud Firestore "(except persistence)".
2. **Firebase JS SDK source** (main branch): `persistentLocalCache()` is IndexedDB-backed (`IndexedDbOfflineComponentProvider`); the RN platform folder has serializers/connection only — **no persistence implementation exists for React Native**.
3. **GitHub issue firebase/firebase-js-sdk#7947** (open since Jan 2024, 23 comments): runtime error — `FirebaseError: [code=unimplemented]: This platform is either missing IndexedDB or is known to have an incomplete implementation. Offline persistence has been disabled.` The SDK **silently falls back to the memory cache**.
...
**If the owner accepts session-scoped offline (recommended for MVP):**
...
**If durable offline is required (restart-safe):**
...
**Install rule of thumb:** every package with native code → `npx expo install` (picks the SDK 57 pin). Pure-JS packages (react-navigation, firebase, xlsx, zustand) → any current version.
...
---
*Stack research for: Money Tracking (Expo Go React Native expense/income tracker)*
*Researched: 2026-08-06*
